jsPsych.plugins["flicker"] = (function() {
	...
	plugin.info = {
		...
		parameters: {
			...
			brightness_boost: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "Brightness boost",
				default: 0,
				description: "Brightness boost to both patches"
			},
			std: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "STD",
			...
			for (i=0; i<trial.num_frames; i++) {
				for (j=0; j<4; j++) {
						if (trial.bright_side=='right' && trial.signal_presence) {
							trial.luminance[0][j][i] = p.round(p.randomGaussian(128+trial.brightness_boost,trial.std))
							trial.luminance[1][j][i] = p.round(p.randomGaussian(128+trial.brightness_boost+trial.delta,trial.std))
					} else if (trial.bright_side=='left' && trial.signal_presence){
						trial.luminance[0][j][i] = p.round(p.randomGaussian(128+trial.brightness_boost+trial.delta,trial.std))
						trial.luminance[1][j][i] = p.round(p.randomGaussian(128+trial.brightness_boost,trial.std))
