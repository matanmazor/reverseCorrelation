jsPsych.plugins["flicker"] = (function() {
	...
	plugin.trial = function(display_element, trial) {
		...
		p.setup = function() {
			...
			for (i=0; i<trial.num_frames; i++) {
				if (trial.bright_side=='right' && trial.signal_presence) {
					trial.luminance[0][i] = p.round(p.randomGaussian(128,trial.std))
					trial.luminance[1][i] = p.round(p.randomGaussian(128+trial.delta,trial.std))
			} else if (trial.bright_side=='left' && trial.signal_presence){
				trial.luminance[0][i] = p.round(p.randomGaussian(128+trial.delta,trial.std))
				trial.luminance[1][i] = p.round(p.randomGaussian(128,trial.std))
		...
		function presentStimulus() {
			p.background(128);
			// draw left square
			p.fill(trial.luminance[0][p.frameCount-trial.lff]);
			p.rect(p.width/2-50, p.windowHeight/2-25, 50, 50);
			// draw right square
			p.fill(trial.luminance[1][p.frameCount-trial.lff]);
			p.rect(p.width/2, p.windowHeight/2-25, 50, 50);
		}
		...
		function rateConfidence() {
			...
			// present choices on top of everything
			presentChoices()
		}
