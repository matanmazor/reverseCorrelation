jsPsych.plugins["flicker"] = (function() {
	...
	plugin.info = {
		...
		parameters: {
			...
			special_side: {
				...
				pretty_name: "Special side",
				default: 'right',
				description: "The side of the special patch (left or right)"
			},
			delta: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "Delta",
				default: 5,
				description: "Mean difference in hue"
			},
			mu: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "Mu",
				default: 5,
				description: "Mean hue"
			},
			boost: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "Hue boost",
				default: 0,
				description: "Hue boost to both patches"
			},
			std: {
				type: jsPsych.plugins.parameterType.FLOAT,
				pretty_name: "STD",
				default: 10,
				description: "Standard deviation of hue levels"
			},
			fixation_duration: {
				type: jsPsych.plugins.parameterType.FLOAT,
	...
	plugin.trial = function(display_element, trial) {
		...
		const bg = polar2rgb(Cstar,trial.mu,lab_origin)
		trial.special_side =  Math.random()>0.5? 'right':'left';
		...
		p.setup = function() {
			...
			// get the hue of the right and left squares as a function of trial
			// parameters:
			trial.hue = [[[],[],[],[]],[[],[],[],[]]] //left,right
			for (i=0; i<trial.num_frames; i++) {
				for (j=0; j<4; j++) {
						if (trial.special_side=='right' && trial.signal_presence) {
							trial.hue[0][j][i] = p.randomGaussian(trial.mu+trial.boost,trial.std)
							trial.hue[1][j][i] = p.randomGaussian(trial.mu+trial.boost+trial.delta,trial.std)
					} else if (trial.special_side=='left' && trial.signal_presence){
						trial.hue[0][j][i] = p.randomGaussian(trial.mu+trial.boost+trial.delta,trial.std)
						trial.hue[1][j][i] = p.randomGaussian(trial.mu+trial.boost,trial.std)
		...
		function presentStimulus() {
			...
			p.strokeWeight(1)
			p.fill(bg)
			p.rect(p.width/2-150,p.windowHeight/2-35,300,76)
			p.pop()

			for (j=0; j<4; j++) { //iterate over squares: external to internal
				// draw left squares
				p.fill(polar2rgb(Cstar,trial.hue[0][j][p.frameCount-trial.lff],lab_origin));
				p.rect(p.width/2-139+(j*14), p.windowHeight/2-25, 14, 56);
				// draw right squares
				p.fill(polar2rgb(Cstar,trial.hue[1][j][p.frameCount-trial.lff],lab_origin));
				p.rect(p.width/2+139-14-(j*14), p.windowHeight/2-25, 14, 56);
			}
			p.fill(255)
			p.rect(p.width/2-12, p.height/2-2, 24, 4);
			p.rect(p.width/2-2, p.height/2-12, 4, 24);
		...
		function rateConfidence() {
			...
			if (trial.present_circle==1 && trial.confidence==undefined) {
				...
				color = [150,150,150].map((e,i)=>p.round(e+confidence*[60,60,60][i]))
				p.fill(color)
				p.strokeWeight(0)
				p.circle(p.width/2,p.height/2, trial.diameter)
			} else if (trial.confidence!==undefined) {
				color = [150,150,150].map((e,i)=>p.round(e+trial.confidence*[60,60,60][i]))
				p.fill(color)
		...
		p.draw = function() {
			...
			if (p.millis()<trial.fixation_duration) {
				...
				trial.status = 'presenting fixation'
					...
					// data saving
					var trial_data = {
						special_side: trial.special_side,
