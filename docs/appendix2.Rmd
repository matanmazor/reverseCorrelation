...
As expected, detection zROC slopes were significantly shallower than 1  (`r e3$detection_zROC%>%pull(logslope)%>%t.test%>%apa_print()%>%'$'(full_result)`), whereas discrimination zROC slopes were significantly not (`r e3$discrimination_zROC%>%pull(logslope)%>%t.test%>%apa_print()%>%'$'(full_result)`). Furthermore, a first-order SDT model fitted the data significantly better for the detection task than for the discrimination (difference in $R^2$ for the two tasks: `r e3$zROC_contrast_rsquare%>%pull(fit_diff)%>%t.test()%>%apa_print()%>%'$'(full_result)`). 
...
e3_st <- e3_st %>%
  ...
  plotDiscriminationKernels('exp3_standard',c(0,440),c(-0.4,0.8))
...
e3_st <- e3_st %>%
  ...
  plotDetectionSignalKernels('exp3_standard',c(0,440),c(-0.4,0.7))
...
As expected, detection zROC slopes were significantly shallower than 1  (`r e4$detection_zROC%>%pull(logslope)%>%t.test%>%apa_print()%>%'$'(full_result)`), whereas discrimination zROC slopes were significantly not (`r e4$discrimination_zROC%>%pull(logslope)%>%t.test%>%apa_print()%>%'$'(full_result)`). Furthermore, a first-order SDT model fitted the data significantly better for the detection than for the discrimination task (difference in $R^2$ for the two tasks: `r e4$zROC_contrast_rsquare%>%pull(fit_diff)%>%t.test()%>%apa_print()%>%'$'(full_result)`). 
...
e4_st <- e4_st %>%
  getDiscriminationKernels() %>%
  contrastDiscriminationKernels() %>%
  plotDiscriminationKernels('exp4_standard',c(0,440),c(-0.4,0.8))
...
e4_st <- e4_st %>%
  getDetectionSignalKernels()%>%
  contrastDetectionSignalKernels()%>%
  plotDetectionSignalKernels('exp4_standard',c(0,440),c(-0.4,0.7))
...
```{r ch2-exp2-PD-RC, echo=FALSE, message=FALSE, warning=FALSE, fig.cap='(ref:caption2)'}
