...
e3_st$discRCdf <- e3$demeaned_df %>%
  ...
  dplyr::select(subj_id,timepoint,obj_side,side,eccentricity,confidence, trial,evidence, time, correct) %>%
  group_by(subj_id) %>%
...
e3_st$detectionRCdf <- e3$demeaned_df %>%
  filter(detection==1 & brightness_boost==0)  %>%
  dplyr::select(subj_id,time,side,eccentricity,confidence, trial,evidence, response) %>%
  group_by(subj_id,time,eccentricity,confidence, trial,evidence, response)%>%
  summarise(evidence=evidence[side==0]+evidence[side==1])%>%
